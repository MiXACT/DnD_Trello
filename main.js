...
;// CONCATENATED MODULE: ./src/js/cardFactory.js
...
var CardFactory = /*#__PURE__*/function () {
  ...
  _createClass(CardFactory, [{
    ...
    key: "addCard",
    value: function addCard(btn) {
      var text = btn.parentNode.querySelector('.textField').value;
      ...
      var cardCloseCross = document.createElement('BUTTON');
      cardCloseCross.setAttribute('class', 'cardCross');
      card.appendChild(cardCloseCross);
    }
  }, {
    key: "disCard",
    value: function disCard(btn) {
      btn.parentNode.remove();
    }
  }, {
    key: "delCard",
    value: function delCard(btn) {
      btn.parentNode.remove();
    }
  }]);
  return CardFactory;
...
var workPanel = document.querySelector('.work_panel');

// загрузка данных из хранилища
var pageStorage = localStorage.getItem('page');
if (pageStorage) {
  var children = workPanel.children;
  for (var node = children.length - 1; node >= 0; node -= 1) {
...
var card = new CardFactory();
workPanel.addEventListener('click', function (e) {
  if (e.target.className === 'btn todo_btn' || e.target.className === 'btn progress_btn' || e.target.className === 'btn done_btn') card.newCard(e.target);else if (e.target.className === 'addCardBtn') card.addCard(e.target);else if (e.target.className === 'crossBtn') card.disCard(e.target);else if (e.target.className === 'cardCross') card.delCard(e.target);
...
workPanel.addEventListener('mousedown', function (e) {
  if (!e.target.classList.contains('card')) return;
  // e.preventDefault();
  actualElement = e.target;
  var parentBlock = actualElement.parentNode;
  var nextEl = actualElement.nextElementSibling;
  var elHeight = actualElement.offsetHeight;

  // определение сдвига для позиционирования курсора в месте захвата карточки
  var shiftX = e.clientX - actualElement.getBoundingClientRect().left;
  var shiftY = e.clientY - actualElement.getBoundingClientRect().top;

  // изменение позиционирования эл-та на absolute
  actualElement.classList.remove('card');
  actualElement.classList.add('dragged');
  actualElement.style.height = "".concat(elHeight, "px");
  ...
  function moveAt(pageX, pageY) {
    actualElement.style.left = "".concat(pageX - shiftX, "px");
    actualElement.style.top = "".concat(pageY - shiftY, "px");
  }

  // ф-ия обработчика события mousemove
  // eslint-disable-next-line
  function onMouseMove(e) {
    var elUnder = getElementUnderCursor(e.pageX, e.pageY);
    moveAt(e.pageX, e.pageY);
    var spaceSize = actualElement.offsetHeight;
    freeSpace.style.height = "".concat(spaceSize, "px");
    if (elUnder.classList.contains('card')) {
      freeSpace.remove();
      var cardCentre = elUnder.getBoundingClientRect().bottom - elUnder.offsetHeight / 2;
      if (cardCentre > e.pageY) elUnder.parentNode.insertBefore(freeSpace, elUnder);else elUnder.parentNode.insertBefore(freeSpace, elUnder.nextSibling);
    } else if ((elUnder.classList.contains('block') || elUnder.closest('.block')) && elUnder.closest('.block').children.length === 1) {
      freeSpace.remove();
  ...
  document.onmouseup = function (e) {
    var mouseUpItem = getElementUnderCursor(e.pageX, e.pageY);
